...
import com.deadvikingstudios.bygul.model.entities.EntityStructure;
...
import com.deadvikingstudios.bygul.model.entities.humanoids.EntityHumanoid;
...
public class World
{
    ...
    private List<EntityStructure> structuresToRemove = new ArrayList<EntityStructure>();
    ...
    public void init()
    {
        this.currentBuildStructure = new StructureIsland(0,0,0);
...
//            if(currentIsland.getTile(x,0,z) != Tile.NorseTiles.tileAir)
//            {
//                i--;
//            }
//            else
...
//        for (int i = -30; i < 30; i++)
//        {
//            TaskEventHandler.createTask(Faction.get("player"), new TaskPlaceTile(currentBuildStructure, Tile.NorseTiles.tileStoneCliff, new Vector3i(i,0,-15), i));
//        }
        int n = 20;
        for (int j = 0; j < 30; j++)
        {
            for (int i = -n; i < n; i++)
            {
//                TaskEventHandler.createTask(Faction.get("player"),new TaskPlaceTile(this.currentBuildStructure, NorseTiles.tileStoneCliff, new Vector3i(i,j,-15), 0));
            }
            n--;
        }
    }
    ...
    {
        for (EntityStructure structEnt : this.structures)
        {
            if(structEnt.getStructure().getParent() == structure)
            {
                structEnt.getStructure().setParent(null);
            }
            if(structEnt.getStructure() == structure)
            {
                this.structuresToRemove.add(structEnt);
            }
        }
    }
...
//        Logger.debug("fizz");
    ...
    public void update()
    {
        structures.removeAll(structuresToRemove);
    ...
    //FIXME: currently not morking correctly
     ...
     * @param byPlayer
     */
    public boolean setTileAt(Structure struct, @NotNull Tile tile, int x, int y, int z, int metadata, boolean byPlayer)
    {
        //TODO: If I know the structure, I should use that structure's coordinates
