package com.deadvikingstudios.bygul.model.entities;

import com.deadvikingstudios.bygul.model.entities.ai.tasks.Task;
...
//                doMovement();
    ...
    protected void doMovement()
    {
        if(pathThread == null && path == null)
        {
            ...
            {
                if (World.getCurrentWorld().getCurrentBuildStructure().getTile(destination.x, destination.y - 1, destination.z) == Tile.Tiles.tileAir)
                    return;
                for (int i = 0; i <= this.height; i++)
                {
                    if (World.getCurrentWorld().getCurrentBuildStructure().getTile(destination.x, destination.y + i, destination.z) != Tile.Tiles.tileAir)
                        return;
                }
...
//                if(World.getCurrentWorld().currentIsland.getTile(vec.x, vec.y, vec.z) != Tile.NorseTiles.tileAir)
                if(counter % 10 == 0)
                {
                    this.recalcPath();
            ...
            if(this.destination != null)
            {
                ...
                {
//                    World.getCurrentWorld().setTileAt(null, Tile.NorseTiles.tileLeaves, this.destination.x, this.destination.y, this.destination.z, 0, true);
