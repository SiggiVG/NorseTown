        ...
        for (int i = -1; i < 2; i++)
        {
            for (int j = -2; j < 2; j++)
            {
                for (int k = -1; k < 2; k++)
                {
                    ...
                    if(World.getCurrentWorld().getTileAt(task.position.x+i, task.position.y+j-1, task.position.z+k).isSolidCuboid())
                    {
                        boolean flag = true;
                        for (int l = 0; l < entityHeight; l++)
                        {
                            if(World.getCurrentWorld().getTileAt(task.position.x+i, task.position.y+j+l, task.position.z+k).isAir())
                            {
                                flag = false;
                            }
                        }
